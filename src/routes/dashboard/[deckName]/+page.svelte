...
<div class="flex items-start justify-between gap-4">
	...
	{#if deckMeta.archidektLink && deckMeta.archidektLink !== '-'}
		<CommanderPreview archidektUrl={deckMeta.archidektLink} summary={deckMeta.summary ?? ''} />
	{/if}
</div>
...
<div class="grid gap-4 md:grid-cols-3">
	...
	<div class="p-4 rounded-xl bg-surface-800 border border-surface-700/60 space-y-1">
		<div class="text-xs uppercase tracking-wide text-surface-400">Table fun (others)</div>
		<div class="text-lg font-semibold">
			{fmtNum(stats.avgFunOthers)}
		</div>
	</div>
	<div class="p-4 rounded-xl bg-surface-800 border border-surface-700/60 space-y-2">
		<div class="text-xs uppercase tracking-wide text-surface-400">Fun when you win</div>
		<div class="text-lg font-semibold">
	...
	<div class="p-4 rounded-xl bg-surface-800 border border-surface-700/60 space-y-2">
		<div class="text-xs uppercase tracking-wide text-surface-400">Fun when you lose</div>
		<div class="text-lg font-semibold">
	...
	<div class="p-4 rounded-xl bg-surface-800 border border-surface-700/60 space-y-2">
		...
		<div class="mt-1 h-2 rounded-full bg-surface-700/70 overflow-hidden flex">
			...
			<div
				...
				style={`width: ${Math.min(stats.expectedWinrate, stats.winRate)}%;`}
			/>
		...
		<div class="text-xs text-surface-400">
			{#if stats.winRate - stats.expectedWinrate > 0}
				You&#39;re winning {(stats.winRate - stats.expectedWinrate).toFixed(1)} percentage points more
				than a perfectly fair deck.
			{:else if stats.winRate - stats.expectedWinrate < 0}
				You&#39;re winning {Math.abs(stats.winRate - stats.expectedWinrate).toFixed(1)} percentage points
					...
					{#each games.slice(-30).reverse() as g}
						<tr class="bg-surface-900 hover:bg-surface-800/40 border-b border-surface-800/60">
							<td class="px-4 py-2">
								{#if g.winner === 1}
									<span
										class="text-xs rounded-full px-2 py-1 bg-success-600/40 text-success-200 border border-success-500/60"
									>
										Win
									</span>
								{:else}
									<span
										class="text-xs rounded-full px-2 py-1 bg-error-600/40 text-error-200 border border-error-500/60"
									>
										Loss
									</span>
								{/if}
